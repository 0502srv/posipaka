"""Security headers middleware для FastAPI (з CSP nonces — audit 102.14)."""
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    Audit 102.14: CSP nonces для inline scripts замість unsafe-inline.
    Nonce доступний у шаблонах через request.state.csp_nonce.
    """

    async def dispatch(self, request: Request, call_next: Any):
        # Генеруємо nonce для кожного запиту (audit 102.14)
