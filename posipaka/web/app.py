...
def create_app(
    ...
    # CORS (секція 102.18 MASTER.md) — з підтримкою custom domains
    import os
    ...
    async def login_submit(request: Request):
        ...
        if auth.verify_password(str(password), client_ip):
            # Bugfix 100.3: session fixation — invalidate old session
    ...
    # ─── Setup Wizard (Web) — секція 10.2 MASTER.md ──────────────────
    ...
    # Audit 102.18: Explicit logout API endpoint
    ...
    # ─── GDPR: Data Export & Delete (секція 102.13 MASTER.md) ─────
